...
# DEVs: what do I need to know? 

## Prereqs  

**Python**: 3.11  
**Requirements.txt**: Fire for parsing args to CLI, Gooey for GUI, PyInstaller for standalone EXE (at the cost of being considered a trojan by a few incorrect AVs..)  

## Algorithm

Iterates over logs, identifies dates and timestamps using regexps, calculates delta per-log, adds them all up. That's it.
...
This code does just that, using basic Python code + two external tools ([Fire](https://github.com/google/python-fire) to turn the code into a CLI automatically, [Gooey](https://github.com/chriskiehl/Gooey) to turn that into a GUI automatically).   

Exe compilation for more recent versions (as of 2024-03-16) done via pyinstaller: `pyinstaller --onefile --windowed sc_main.py`  
